...
app.get('/health', (req, res) => {
  res.json({ status: 'OK', message: 'Mock API is running with real data' });
});

// Get all properties
app.get('/api/properties', (req, res) => {
  console.log('Serving real property data from mock API');
  res.json(properties);
});
...
app.get('/api/properties/:id', (req, res) => {
  ...
  console.log(`Serving property ${id} from mock API`);
  res.json(property);
});
...
app.get('/api/properties/:id/images', (req, res) => {
  ...
  console.log(`Serving ${property.images?.length || 0} images for property ${id}`);
  res.json(property.images || []);
});

// Get property amenities
app.get('/api/properties/:id/amenities', (req, res) => {
  const { id } = req.params;
  const property = properties.find(p => p.id === id);
  ...
  console.log(`Serving ${property.amenities?.length || 0} amenities for property ${id}`);
